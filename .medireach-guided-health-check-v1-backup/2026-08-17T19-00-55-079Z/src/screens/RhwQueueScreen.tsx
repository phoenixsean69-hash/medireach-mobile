...
export default function RhwQueueScreen() {
  ...
  const openClinicalCapture=(row:RhwCareRequest)=>{
    router.push({
      ...
      params:{
        ...
        patientUserId:row.patient?.userId??row.createdByUserId??row.requesterUserId??"",
        patientName:patientDisplayName(row.patient,row.patientId),
        facilityId:row.facilityId??row.patient?.facilityId??"",
        sourceVoiceNoteFileId:row.voiceNoteFileId??row.voiceFileId??"",
        sourceImageFileId:"",
      },
    } as any);
  };
                ...
                {open ? (
                  <View style={styles.detail}>
                    ...
                    <View style={styles.actions}>
                      ...
                      {mine&&!["completed","closed","cancelled"].includes(status) ? (

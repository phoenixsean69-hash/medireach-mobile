...
function isRelevant(row:RhwCareRequest|RhwSosAlert, actor:RhwActor) {
  const assigned = clean(row.assignedUserId);
  const rowFacility = clean(row.facilityId);
  const myFacility = clean(actor.profile?.facilityId);

  if (assigned === actor.userId) return true;
  if (assigned && assigned !== actor.userId) return false;
  if (myFacility && rowFacility) return myFacility === rowFacility;
  return true;
}

async function decorateCare(rows:RhwCareRequest[]) {
  const cache = new Map<string,RhwPatientSummary|null>();
  return Promise.all(rows.map(async row => {
    if (!cache.has(row.patientId)) cache.set(row.patientId, await patientFor(row.patientId));
    return {...row, patient:cache.get(row.patientId) ?? null};
  }));
}

async function decorateSos(rows:RhwSosAlert[]) {
  const cache = new Map<string,RhwPatientSummary|null>();
  return Promise.all(rows.map(async row => {
    if (!cache.has(row.patientId)) cache.set(row.patientId, await patientFor(row.patientId));
    return {...row, patient:cache.get(row.patientId) ?? null};
...
export async function listRhwCareRequests() {
  ...
  return decorateCare(
    (result.rows as unknown as RhwCareRequest[]).filter(row => isRelevant(row,actor))
  );
}
...
export async function listRhwSosAlerts() {
  ...
  return decorateSos(
    (result.rows as unknown as RhwSosAlert[]).filter(row => isRelevant(row,actor))
  );
}

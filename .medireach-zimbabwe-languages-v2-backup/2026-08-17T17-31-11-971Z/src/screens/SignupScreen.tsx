...
import {
    ...
    Eye, EyeOff, HeartPulse, Languages, ShieldCheck, Stethoscope, UserRound,
...
import {
    ...
    CLINICAL_SPECIALTIES, DEPARTMENTS_WARDS, DISABILITIES,
...
function Field({
  ...
  return (
    <View style={styles.field}>
      <Text style={styles.label}>{t(label)}{required ? " *" : ""}</Text>
...
export default function SignupScreen() {
  ...
  const [languageDetection, setLanguageDetection] =
    useState<LocalLanguageDetection>({
      status: "detecting",
      language: "English",
      areaLabel: "",
    });
  const languageWasManuallySelected = useRef(false);

  const language =
    normalizeSignupLanguage(form.preferredLanguage);

  const t = (text: string) =>
    signupT(text, language);
  ...
  const chooseLanguage = (nextLanguage: SignupLanguage) => {
    languageWasManuallySelected.current = true;
    set("preferredLanguage", nextLanguage);
  };
  ...
  useEffect(() => {
    ...
    const detect = async () => {
      ...
      setLanguageDetection(detected);

      if (
        detected.status === "detected" &&
        !languageWasManuallySelected.current
      ) {
        setForm((current) => ({
          ...current,
  ...
  const next = () => {
    ...
    if (step === 1) {
      if (!form.firstName.trim() || !form.lastName.trim()) {
        Alert.alert(t("Personal details"), t("First name and last name are required."));
        return;
      }
      if (!form.dateOfBirth || !form.gender) {
        Alert.alert(t("Personal details"), t("Date of birth and gender are required."));
        return;
      }
      if (!form.phone.trim()) {
        Alert.alert(t("Contact details"), t("Phone number is required."));
        return;
      }
      if (!form.province || !form.district || !form.townVillage.trim()) {
        Alert.alert(t("Location details"), t("Province, district and town / village are required."));
        return;
      }
    }
  ...
  const submit = async () => {
    ...
    if (issue) {
      Alert.alert(t("Check your details"), t(issue));
      return;
    }
    ...
    try {
      ...
      Alert.alert(t("Signup failed"), error?.message ?? t("MediReach could not create your account."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SignupLanguageProvider language={language}>
    ...
    >
      ...
      >
        ...
        <View style={styles.localeCard}>
          <View style={styles.localeHeader}>
            <View style={styles.localeIcon}>
              <Languages size={18} color={colors.charcoal} />
            </View>

            <View style={styles.localeCopy}>
              <Text style={styles.localeTitle}>
                {languageDetection.status === "detecting"
                  ? t("Detecting local language...")
                  : languageDetection.status === "detected"
                    ? `${t("Detected area")}: ${languageDetection.areaLabel || "Zimbabwe"}`
                    : languageDetection.status === "permission_denied"
                      ? t("Location permission was not granted. Choose a language manually.")
                      : languageDetection.status === "outside_zimbabwe"
                        ? t("English is being used because this area is outside Zimbabwe or could not be matched.")
                        : t("Could not detect the local area. Choose a language manually.")}
              </Text>

              {languageDetection.status === "detected" ? (
                <Text style={styles.localeSubtitle}>
                  {t("Using local language")}: {signupLanguageLabel(language, language)}. {t("Language can be changed at any time.")}
                </Text>
              ) : null}
            </View>
          </View>
              ...
              return (
                ...
                >
                  <Text style={[styles.languageChipText, active && styles.languageChipTextActive]}>
                    {signupLanguageLabel(item, language)}
                  </Text>
                </Pressable>
              );
            })}
        ...
        <View style={styles.card}>
          {step === 0 && (
            <>
              ...
              <Text style={styles.subtitle}>{t("Tell us how you will use MediReach.")}</Text>
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={styles.roleTextWrap}>
                        ...
                        <Text style={styles.roleSubtitle}>{t(item.subtitle)}</Text>
                      </View>
              ...
              <View style={styles.noteBox}>
                ...
                <Text style={styles.noteText}>
                  {t("Hospital administrator and MediReach administrator accounts are invitation-only.")}
                </Text>
              </View>
            </>
          )}
          ...
          {step === 1 && (
            <>
                ...
                <View style={styles.genderRow}>
                  {(["Male","Female"] as const).map((gender) => {
                    ...
                    return (
                      ...
                      >
                        <Text style={[styles.genderText, active && styles.genderTextActive]}>{t(gender)}</Text>
                      </Pressable>
                    );
                  })}
                </View>
              ...
              <SinglePicker
                ...
                options={LANGUAGES}
                onChange={(v) => chooseLanguage(normalizeSignupLanguage(v))}
                allowCustom={false}
                required
              />
              ...
              <SinglePicker
                ...
                onChange={(v) => {
                  const provinceChanged = v !== form.province;
                  set("province", v);
              ...
              <SinglePicker
                label="District"
                value={form.district}
                options={DISTRICTS_BY_PROVINCE[form.province] ?? []}
              ...
              <LocationPickerField
                ...
                onChange={(latitude, longitude) => {
                  ...
                  set("locationLongitude", longitude);
                }}
                onClear={() => {
                  set("locationLatitude", null);
                  set("locationLongitude", null);
                }}
              />
            </>
          )}
          ...
          {step === 2 && (
            <>
              <Text style={styles.title}>{form.role === "citizen" ? t("Health details") : t("Professional details")}</Text>
              <Text style={styles.subtitle}>{t("Questions are tailored to your MediReach account type.")}</Text>
          ...
          {step === 3 && (
            <>
              <Text style={styles.title}>{t("Emergency & security")}</Text>
              <Text style={styles.subtitle}>{t("Finish your MediReach account.")}</Text>
              ...
              >
                ...
                <Text style={styles.confirmText}>
                  {t("I confirm that the information I provided is accurate and belongs to me.")}
                </Text>
              </Pressable>
            </>
          )}
          ...
          <View style={styles.actions}>
            ...
            >
              ...
              <Text style={styles.secondaryText}>{step > 0 ? t("Back") : t("I have an account")}</Text>
            </Pressable>
        ...
        <Text style={styles.stepText}>{signupStepText(step + 1, language)}</Text>
      </ScrollView>
      </KeyboardAvoidingView>
    </SignupLanguageProvider>
  );
...
const styles = StyleSheet.create({
  ...
  localeIcon: {
    width: 36, height: 36, borderRadius: 10, backgroundColor: colors.surfaceSoft,
    alignItems: "center", justifyContent: "center",
  },
  localeCopy: { flex: 1 },
  localeTitle: {
    fontFamily: fonts.bold, color: colors.text, fontSize: 10, lineHeight: 15,
  },
  localeSubtitle: {
    marginTop: 2, fontFamily: fonts.regular, color: colors.muted,
  ...
  languageChipActive: {
    borderColor: colors.charcoal, backgroundColor: colors.charcoal,
  },
  languageChipText: {
    fontFamily: fonts.bold, color: colors.text, fontSize: 8, textAlign: "center",
  },
